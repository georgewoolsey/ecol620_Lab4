...
np_temp <- c(4,8) %>% 
  purrr::map(get_np_fn, my_class_str = c("evergreen forest"))

```

<span style="color: teal;">
The NLCD raster of Fort Collins contains **`r np_temp[[1]]`** patches of evergreen forest (based on NLCD classification) using a 4-neighborhood rule and **`r np_temp[[2]]`** patches of evergreen forest (based on NLCD classification) using a 8-neighborhood rule.
</span>
